...
export interface PromptTemplateRecord { id: string; item_id: string; source_language: string; raw_text_snapshot: string; marked_text: string; slots: PromptTemplateSlot[]; status: string; review_status: string; review_notes?: string; reviewed_at?: string; analysis_confidence?: number; analysis_notes?: string; prompt_source_extracted: boolean; prompt_source_strategy?: string; prompt_source_original_length?: number; prompt_source_prepared_length?: number; created_at: string; updated_at: string }
export interface PromptTemplateBundle { template?: PromptTemplateRecord; sessions: PromptGenerationSessionRecord[] }
export interface PromptImageGenerationOptions { resolution?: string; aspect_ratio?: string; image_count?: number; style?: string; strength?: number }
